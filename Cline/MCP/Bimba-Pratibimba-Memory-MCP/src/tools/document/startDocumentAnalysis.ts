...
export async function handleStartDocumentAnalysis(dependencies: ToolDependencies, args: any) {
  try {
    ...
    // Update document status and store targetCoordinate
    ...
    // Include graphData in the response for enhanced Bimba awareness
    return {
      content: [{
        type: "text",
        text: JSON.stringify({
          ...
          graphData: validatedArgs.graphData || { nodes: [], edges: [] },
          targetCoordinate: validatedArgs.targetCoordinate
        }, null, 2)
      }]
    };
