...
- [ ] Architecture supports all functional requirements in the EFDD for **{Development Name}** (located at `{epi-logos-memory-root}/{PhilosophicalLayer}/{Subsystem}/Developments/{DevelopmentName}/1_feature_definition/efdd.md`)
...
- [ ] All required integrations for **{Development Name}** are accounted for, referencing `{epi-logos-memory-root}/Epi-Logos System (Technology)/Integration_Patterns.md`
- [ ] User journeys for **{Development Name}** are supported by the technical architecture

### 1.2 Non-Functional Requirements Alignment

- [ ] Performance requirements for **{Development Name}** are addressed with specific solutions, referencing foundational principles in `{epi-logos-memory-root}/Epi-Logos Project (Philosophy)/NFRs.md` and specific NFRs in the EFDD for **{DevelopmentName}**.
- [ ] Scalability considerations for **{Development Name}** are documented with approach, aligned with **{PhilosophicalLayer}** principles and `{epi-logos-memory-root}/Epi-Logos System (Technology)/Scalability_Performance_Guidelines.md`.
- [ ] Security requirements for **{Development Name}** have corresponding technical controls, referencing `{epi-logos-memory-root}/Epi-Logos Project (Philosophy)/Security_Principles.md` and `{epi-logos-memory-root}/Epi-Logos System (Technology)/Security_Guidelines.md`.
...
- [ ] Platform/language requirements for **{Development Name}** are followed, consistent with `{epi-logos-memory-root}/Epi-Logos System (Technology)/TechStack.md`
- [ ] Infrastructure constraints for **{Development Name}** are accommodated, referencing `{epi-logos-memory-root}/Epi-Logos System (Technology)/Deployment_Strategy.md`
- [ ] Third-party service constraints for **{Development Name}** are addressed, referencing `{epi-logos-memory-root}/Epi-Logos System (Technology)/ThirdPartyIntegrations.md`
- [ ] Organizational technical standards (e.g., from `{epi-logos-memory-root}/Epi-Logos System (Technology)/Coding_Standards.md`) are followed for **{Development Name}**

## 2. ARCHITECTURE FUNDAMENTALS (for {Development Name})

### 2.1 Architecture Clarity

- [ ] Architecture for **{Development Name}** is documented with clear diagrams in `{epi-logos-memory-root}/{PhilosophicalLayer}/{Subsystem}/Developments/{DevelopmentName}/architecture/diagrams/`
- [ ] Major components and their responsibilities for **{Development Name}** are defined, consistent with `{epi-logos-memory-root}/Epi-Logos System (Technology)/ComponentLibrary.md` and detailed in `{epi-logos-memory-root}/{PhilosophicalLayer}/{Subsystem}/Developments/{DevelopmentName}/architecture/components/`
...
- [ ] Data flows for **{Development Name}** are clearly illustrated, aligning with the overall Epi-Logos data philosophy (`{epi-logos-memory-root}/Epi-Logos System (Technology)/Data_Management_Strategy.md`).
- [ ] Technology choices for each component of **{Development Name}** are specified, referencing `{epi-logos-memory-root}/Epi-Logos System (Technology)/TechStack.md`.
...
- [ ] Cross-cutting concerns (logging, auth, etc.) for **{Development Name}** are properly addressed, referencing `{epi-logos-memory-root}/Epi-Logos System (Technology)/Cross_Cutting_Concerns.md`.

### 2.3 Design Patterns & Best Practices

- [ ] Appropriate design patterns are employed for **{Development Name}**, referencing `{epi-logos-memory-root}/Epi-Logos System (Technology)/Architectural_Patterns/`.
...
- [ ] Pattern usage is documented and explained in `{epi-logos-memory-root}/{PhilosophicalLayer}/{Subsystem}/Developments/{DevelopmentName}/architecture/`.
...
- [ ] Selected technologies meet all requirements for **{Development Name}**, consistent with `{epi-logos-memory-root}/Epi-Logos System (Technology)/TechStack.md`.
- [ ] Technology versions for **{Development Name}** are specifically defined (not ranges), as per `{epi-logos-memory-root}/Epi-Logos System (Technology)/TechStack.md`.
...
- [ ] UI framework and libraries for **{Development Name}** are specifically selected, aligned with Shakti principles and `{epi-logos-memory-root}/Epi-Logos System (Technology)/5-3_Sakti/Frontend_Patterns.md`. Details in `{epi-logos-memory-root}/{PhilosophicalLayer}/{Subsystem}/Developments/{DevelopmentName}/architecture/front-end-architecture.md`.
- [ ] State management approach for **{Development Name}** is defined, referencing `{epi-logos-memory-root}/Epi-Logos System (Technology)/5-3_Sakti/State_Management_Guidelines.md`.
...
- [ ] API design and standards for **{Development Name}** are defined, aligned with Siva principles and `{epi-logos-memory-root}/Epi-Logos System (Technology)/5-2_Siva/Backend_Patterns.md`. Details in `{epi-logos-memory-root}/{PhilosophicalLayer}/{Subsystem}/Developments/{DevelopmentName}/architecture/api-specs.md`.
- [ ] Service organization and boundaries for **{Development Name}** are clear.
- [ ] Authentication and authorization approach for **{Development Name}** is specified, referencing `{epi-logos-memory-root}/Epi-Logos System (Technology)/Security_Guidelines.md`.
- [ ] Error handling strategy for **{Development Name}** is outlined, referencing `{epi-logos-memory-root}/Epi-Logos System (Technology)/Error_Handling_Guidelines.md`.
- [ ] Backend scaling approach for **{Development Name}** is defined.

### 3.4 Data Architecture (Siva Aspect of {Development Name})

- [ ] Data models for **{Development Name}** are fully defined in `{epi-logos-memory-root}/{PhilosophicalLayer}/{Subsystem}/Developments/{DevelopmentName}/architecture/data-models.md`, consistent with Siva principles and `{epi-logos-memory-root}/Epi-Logos System (Technology)/DataModels.md`.
...
- [ ] Error handling strategy for **{Development Name}** is comprehensive, aligned with **{PhilosophicalLayer}** resilience principles and `{epi-logos-memory-root}/Epi-Logos System (Technology)/Error_Handling_Guidelines.md`.
...
- [ ] Logging strategy for **{Development Name}** is defined, referencing `{epi-logos-memory-root}/Epi-Logos System (Technology)/Observability_Strategy.md`.
...
- [ ] Deployment strategy for **{Development Name}** is defined, referencing `{epi-logos-memory-root}/Epi-Logos System (Technology)/Deployment_Strategy.md`.
...
- [ ] Authentication mechanism for **{Development Name}** is clearly defined, consistent with `{epi-logos-memory-root}/Epi-Logos System (Technology)/Security_Guidelines.md` and `{epi-logos-memory-root}/Epi-Logos Project (Philosophy)/Security_Principles.md`.
...
- [ ] Data encryption approach (at rest and in transit) for **{Development Name}** is specified, aligned with `{epi-logos-memory-root}/Epi-Logos System (Technology)/Security_Guidelines.md`.
...
- [ ] Coding standards for **{Development Name}** are defined, referencing `{epi-logos-memory-root}/Epi-Logos System (Technology)/Coding_Standards.md`.
- [ ] Documentation requirements for **{Development Name}** are specified.
- [ ] Testing expectations for **{Development Name}** are outlined, referencing `{epi-logos-memory-root}/Epi-Logos System (Technology)/Testing_Strategy.md`.
...
- [ ] Local development environment setup for **{Development Name}** is documented in `{epi-logos-memory-root}/{PhilosophicalLayer}/{Subsystem}/Developments/{DevelopmentName}/docs/dev-environment.md`.
...
- [ ] API documentation standards for **{Development Name}** are defined, consistent with `{epi-logos-memory-root}/{PhilosophicalLayer}/{Subsystem}/Developments/{DevelopmentName}/architecture/api-specs.md`.
- [ ] Architecture documentation requirements for **{Development Name}** are specified, with outputs in `{epi-logos-memory-root}/{PhilosophicalLayer}/{Subsystem}/Developments/{DevelopmentName}/architecture/`.
...
- [ ] Decision records for key choices for **{Development Name}** are included in `{epi-logos-memory-root}/{PhilosophicalLayer}/{Subsystem}/Developments/{DevelopmentName}/architecture/decisions.md`.

## 7. DEPENDENCY & INTEGRATION MANAGEMENT (for {Development Name})

### 7.1 External Dependencies

- [ ] All external dependencies for **{Development Name}** are identified, consistent with `{epi-logos-memory-root}/Epi-Logos System (Technology)/ExternalDependencies.md`.
...
- [ ] All third-party integrations for **{Development Name}** are identified, consistent with `{epi-logos-memory-root}/Epi-Logos System (Technology)/ThirdPartyIntegrations.md`.
...
- [ ] Detailed implementation guidance for **{Development Name}** is provided in `{epi-logos-memory-root}/{PhilosophicalLayer}/{Subsystem}/Developments/{DevelopmentName}/docs/implementation-guidance.md`.
