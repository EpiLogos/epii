...
import { usePulsationAnimation } from './usePulsationAnimation';
...
export function useGraphRendering3D(
  ...
  // Re-enabled pulsation animation with direct Three.js manipulation
  // This avoids physics resets while still providing visual animation
  useEffect(() => {
    // Function to update the pulsation factor and directly update materials
    const updatePulsation = () => {
      ...
      // CRITICAL FIX: Instead of refreshing the entire graph, directly update link materials
      // This avoids physics resets while still allowing pulsation animation
  ...
  const prevHighlightedNodesRef = useRef(highlightedNodes);
  const prevHighlightedLinksRef = useRef(highlightedLinks);
