...
export function createMainTorusWireframe(_nodes: Node[]): THREE.Object3D | null {
  try {
    ...
    // Use the geometry cache to avoid recreating geometries
    const torusGeometry = getTorusGeometry(
      torusRadius,       // radius
...
export function updateTorusWireframes(nodes: Node[], scene: THREE.Scene): void {
  ...
  try {
    ...
    while (torusGroup.children.length > 0) {
      const child = torusGroup.children[0];
    ...
    // Dispatch a custom event to notify that torus wireframes are ready
    // This ensures the animation hook can detect them immediately
    window.dispatchEvent(new CustomEvent('torusWireframesCreated'));
