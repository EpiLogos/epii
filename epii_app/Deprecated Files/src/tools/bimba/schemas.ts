...
export const BimbaKnowingSchema = z.object({
  ...
  limit: z.number().int().min(1).max(100).default(20).describe("Maximum number of results to return (default: 20)."),
...
export const GenerateBimbaEmbeddingsSchema = z.object({
  limit: z.number().int().min(1).max(1000).default(100).describe("Maximum number of nodes to process (default: 100)."),
